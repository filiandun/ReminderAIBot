...
namespace ReminderAIBot.Services.SenderService
{
    public class TelegramSenderService : ISenderService
    {
        ...
        // TODO подумать над тем, как делать многоуровневые кнопки (это больше вопрос к моделям)
        private InlineKeyboardMarkup CreateInlineKeyboard(List<MessageButton> buttons)
        {
            InlineKeyboardMarkup inlineKeyboardMarkup = new InlineKeyboardMarkup();

            foreach (MessageButton button in buttons)
            {
                inlineKeyboardMarkup.AddButton(button.Text, button.CallbackData ?? string.Empty);
            }
